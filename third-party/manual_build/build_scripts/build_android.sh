...
#If the script complains about realpath, do "brew install coreutils"
SCRIPTPATH=$(realpath $REL_SCRIPT_PATH)
CURLPATH="$SCRIPTPATH/../curl"
SSLPATH="$SCRIPTPATH/../openssl"

if [ -z "$NDK_ROOT" ]; then
  echo "Please set your NDK_ROOT environment variable first (export NDK_ROOT=/Users/XXX/android-ndk-r10d/)"
  exit 1
fi
...
if [ $EXITCODE -ne 0 ]; then
	echo "Error running the ssl configure program. You must install the submodules for ssl and curl (git submodule init + git submodule update)"
	cd $PWD
	exit $EXITCODE
fi
...
for p in ${PLATFORMS[*]}; do 
  ...
  cp $SCRIPTPATH/obj/local/$p/libssl.a $SCRIPTPATH/../../openssl/android/$p/libssl.a
  cp $SCRIPTPATH/obj/local/$p/libcrypto.a $SCRIPTPATH/../../openssl/android/$p/libcrypto.a

done

#Copy openssl headers
cp -L -R $SSLPATH/include $SCRIPTPATH/../../openssl/android/

#Copy cURL headers
cp -R $CURLPATH/include $DESTDIR/
rm $DESTDIR/include/curl/.gitignore

rm -rf $SCRIPTPATH/obj
cd $PWD
exit 0
